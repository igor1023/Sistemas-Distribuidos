...
public class App {
    public static void main(String[] args) throws Exception {
        ...
        var outra = Pessoa.newBuilder().setId(2).build();
        var respostaBusca = agendaBlockingStub.buscar(outra);

        if (respostaBusca.getId() == 0){
            IO.println("Nao existe");
        }else{
            System.out.println("Dados da pessoa retornada pelo servidor: " + respostaBusca);
